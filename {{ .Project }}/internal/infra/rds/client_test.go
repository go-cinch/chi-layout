package rds

import (
	"context"
	"reflect"
	"testing"
	"time"

	"github.com/redis/go-redis/v9"
)

type captureCommands struct {
	commands [][]interface{}
}

func (h *captureCommands) DialHook(next redis.DialHook) redis.DialHook { return next }

func (h *captureCommands) ProcessHook(_ redis.ProcessHook) redis.ProcessHook {
	return func(_ context.Context, command redis.Cmder) error {
		h.commands = append(h.commands, append([]interface{}(nil), command.Args()...))
		return nil
	}
}

func (h *captureCommands) ProcessPipelineHook(next redis.ProcessPipelineHook) redis.ProcessPipelineHook {
	return next
}

func testClient(t *testing.T, prefix string, hook *captureCommands) *Client {
	t.Helper()
	client, err := newClient(&redis.Options{Addr: "unused:6379"}, prefix)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = client.Close() })
	client.client.AddHook(hook)
	return client
}

func TestClientPrefixesEveryKeyCommand(t *testing.T) {
	hook := &captureCommands{}
	client := testClient(t, "dev:example:", hook)
	ctx := t.Context()
	client.Set(ctx, "session:id", "session:value", time.Minute)
	client.Get(ctx, "session:id")
	client.GetDel(ctx, "refresh:digest")
	keys := []string{"session:id", "refresh:digest"}
	client.Del(ctx, keys...)
	client.SetNX(ctx, "idempotent:request", "1", time.Minute)
	client.Incr(ctx, "counter:requests")
	script := "return redis.call('GET', KEYS[1])"
	client.Eval(ctx, script, keys, "unchanged:argument")
	client.Ping(ctx)
	if !reflect.DeepEqual(keys, []string{"session:id", "refresh:digest"}) {
		t.Fatalf("caller keys mutated: %v", keys)
	}
	want := [][]interface{}{
		{"set", "dev:example:session:id", "session:value", "ex", int64(60)},
		{"get", "dev:example:session:id"},
		{"getdel", "dev:example:refresh:digest"},
		{"del", "dev:example:session:id", "dev:example:refresh:digest"},
		{"set", "dev:example:idempotent:request", "1", "ex", int64(60), "nx"},
		{"incr", "dev:example:counter:requests"},
		{"eval", script, 2, "dev:example:session:id", "dev:example:refresh:digest", "unchanged:argument"},
		{"ping"},
	}
	if !reflect.DeepEqual(hook.commands, want) {
		t.Fatalf("commands = %#v, want %#v", hook.commands, want)
	}
}

func TestClientNamespaceIsolation(t *testing.T) {
	hook := &captureCommands{}
	for _, prefix := range []string{"dev:example", "prod:example:", "dev:order:"} {
		client := testClient(t, prefix, hook)
		client.Get(t.Context(), "session:same-id")
		client.Del(t.Context(), "session:same-id")

	}
	for index, prefix := range []string{"dev:example:", "prod:example:", "dev:order:"} {
		for offset, key := range []string{"session:same-id", "session:same-id"} {
			if got := hook.commands[index*2+offset][1]; got != prefix+key {
				t.Fatalf("namespace %s command %d key = %v", prefix, offset, got)
			}
		}
	}
}

func TestClientRejectsInvalidPrefixes(t *testing.T) {
	for _, prefix := range []string{"", " ", "auth", ":auth", "dev:", "dev:example::", "dev:example:extra", "dev:*", "dev:a b", "dev:a?", "dev:{auth}", "dev:a\\b"} {
		if client, err := newClient(&redis.Options{}, prefix); err == nil {
			_ = client.Close()
			t.Errorf("accepted prefix %q", prefix)
		}
	}
	hook := &captureCommands{}
	client := testClient(t, " dev:example-service.v2_1: ", hook)
	client.Get(t.Context(), "key")
	if got := hook.commands[0][1]; got != "dev:example-service.v2_1:key" {
		t.Fatalf("normalized key = %v", got)
	}
}
