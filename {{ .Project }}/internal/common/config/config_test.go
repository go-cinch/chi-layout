...
func TestRedisPrefixConfiguration(t *testing.T) {
    cfg, _, err := LoadDir("../../../conf")
    if err != nil { t.Fatal(err) }
    if cfg.Redis.Prefix != "dev:{{ .Scaffold.Project }}:" { t.Fatalf("project prefix: %q", cfg.Redis.Prefix) }
    t.Setenv("SERVICE_REDIS_PREFIX", "prod:orders:")
    cfg, _, err = LoadDir("../../../conf")
    if err != nil { t.Fatal(err) }
    if cfg.Redis.Prefix != "prod:orders:" { t.Fatalf("overridden prefix: %q", cfg.Redis.Prefix) }
}
{{ end -}}
