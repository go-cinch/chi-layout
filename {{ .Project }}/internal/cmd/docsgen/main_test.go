...
`)
	...
	if strings.Contains(generated, "bearerAuth") || strings.Contains(generated, "security:") {
		t.Fatal("authentication added to ordinary routes")
	}
	for _, expected := range []string{
		...
		"User:", "created_at:", "format: date-time", "ErrorResponse:", "error_code:",
...
func TestCodedErrorDescriptions(t *testing.T) {
	root := t.TempDir()
	writeSource(t, root, "conf/http.yml", "http:\n  docs:\n    servers: []\n")
	writeSource(t, root, "common/apperror/error.go", `package apperror
var InvalidBody = New("HTTP_INVALID_BODY", "invalid request body")
`)
	writeSource(t, root, "modules/game/http.go", `package game
import (
    "net/http"
    public "example.com/service/internal/common/apperror"
)
var ErrNotFound = public.New("GAME_NOT_FOUND", "game not found")
type Module struct{}
func (*Module) Name() string { return "/game" }
func (m *Module) HTTP() http.Handler { r := chi.NewRouter(); r.Get("/", m.get); return r }
func (m *Module) get(w http.ResponseWriter, r *http.Request) {
    server.WriteError(w, r, http.StatusBadRequest, public.InvalidBody)
...
`)
	...
	for _, expected := range []string{`description: "invalid request body"`, `description: "game not found"`, "required: [error_code, msg]"} {
