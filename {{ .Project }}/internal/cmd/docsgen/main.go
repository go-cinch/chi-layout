...
type sourcePackage struct {
	...
	relativePath   string
	files          []*ast.File
	selectorErrors map[token.Pos]string
...
func loadPackages(root string) ([]*sourcePackage, error) {
	...
	err = filepath.WalkDir(root, func(path string, entry os.DirEntry, walkErr error) error {
		...
		for _, parsedPackage := range parsed {
			pkg := &sourcePackage{
				name: parsedPackage.Name, selectorErrors: make(map[token.Pos]string), funcs: make(map[string]*ast.FuncDecl),
				types: make(map[string]ast.Expr), errors: make(map[string]string),
				receivers: make(map[string]string),
			}
			pkg.relativePath, _ = filepath.Rel(root, path)
			pkg.relativePath = filepath.ToSlash(pkg.relativePath)
	...
	resolveImportedErrors(packages)
	return packages, nil
}
...
func (p *sourcePackage) collectError(value *ast.ValueSpec) {
	for index, name := range value.Names {
		...
		if functionName(call.Fun) == "New" && messageOK {
			p.errors[name.Name] = message
		}
	}
}

// Resolve error constants using each file's import aliases. Only packages under
// the supplied source root participate; unrelated dependencies are not evaluated.
func resolveImportedErrors(packages []*sourcePackage) {
	for _, pkg := range packages {
		for _, file := range pkg.files {
			imported := make(map[string]*sourcePackage)
			for _, item := range file.Imports {
				...
				for _, candidate := range packages {
					if !strings.HasSuffix(path, "/"+candidate.relativePath) {
						continue
					}
			...
			ast.Inspect(file, func(node ast.Node) bool {
				...
				if target := imported[alias.Name]; target != nil {
					if message := target.errors[selector.Sel.Name]; message != "" {
						pkg.selectorErrors[selector.Pos()] = message
					}
				}
				return true
			})
...
func (g *generator) ensureErrorResponse() {
	...
	g.components["ErrorResponse"] = &schema{
		...
		required:   []string{"error_code", "msg"},
	}
}
...
func errorDescription(pkg *sourcePackage, expression ast.Expr, status int) string {
	if message := pkg.selectorErrors[expression.Pos()]; message != "" {
		return message
	}
