...
func discoverModules(dir, rootImport string, application map[string][]string) ([]moduleSpec, error) {
	...
	for _, entry := range entries {
		...
		spec, found, err := inspectModule(filepath.Join(dir, entry.Name()), rootImport, application)
		if err != nil {
		...
		spec.ImportPath = rootImport + "/" + entry.Name()
...
func inspectModule(dir, rootImport string, application map[string][]string) (moduleSpec, bool, error) {
	fileSet := token.NewFileSet()
	...
	for _, currentPackage := range packages {
		...
		for _, file := range currentPackage.Files {
			...
			if name := assertedModule(file, imports, rootImport); name != "" {
				implementation = name
			}
		}
		if implementation == "" {
		...
		for _, file := range currentPackage.Files {
			...
			for _, declaration := range file.Decls {
				...
				arguments, err := constructorArguments(function.Type.Params, imports, application)
				if err != nil {
					return moduleSpec{}, false, err
				}
				return moduleSpec{Alias: currentPackage.Name, Arguments: arguments}, true, nil
			}
		}
...
func assertedModule(file *ast.File, imports map[string]string, rootImport string) string {
	for _, declaration := range file.Decls {
		generic, ok := declaration.(*ast.GenDecl)
...
func constructorArguments(parameters *ast.FieldList, imports map[string]string, application map[string][]string) ([]string, error) {
	if parameters == nil {
		return nil, nil
	}
...
func generate(modulePath string, specs []moduleSpec) ([]byte, error) {
	...
	source.WriteString(")\n\nfunc (a *Application) generatedModules() []modules.Module {\n")
	if len(specs) == 0 {
		source.WriteString("\treturn nil\n}\n")
	} else {
		source.WriteString("\treturn []modules.Module{\n")
		for _, spec := range specs {
			fmt.Fprintf(&source, "\t\t%s.New(%s),\n", spec.Alias, strings.Join(spec.Arguments, ", "))
		}
		source.WriteString("\t}\n}\n")
	}
