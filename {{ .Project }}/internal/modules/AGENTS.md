...
- Keep the root `HTTPModule`{{ if .Computed.enable_grpc_final }}, `GRPCModule`,{{ end }} and `Module` contracts together in `internal/modules/module.go`.
...
- Use `PATCH /<resource>/{id}` for partial updates. Preserve omitted fields; use pointer fields in HTTP inputs{{ if .Computed.enable_grpc_final }} and Proto `optional` fields{{ end }} to distinguish omission from an explicit empty value.
- Do not impose an `/api` or version prefix inside the module.

## Naming and Pagination

- Follow the [root CRUD naming rules and AIP references](../../AGENTS.md#crud-and-pagination).{{ if .Computed.enable_grpc_final }} RPC requests use `<Method>Request`; list responses use `<Method>Response`. Get/Create/Update return the resource; Delete returns `google.protobuf.Empty`.{{ end }}
...
- Use Go pointers{{ if .Computed.enable_grpc_final }} and Proto `optional` fields{{ end }} to preserve presence. Omitted `p`/`s` select page 1 and size `min(1, maxS)`; explicit values outside the data window return an empty list without querying the database. Format errors return HTTP 400{{ if .Computed.enable_grpc_final }} or gRPC `InvalidArgument`{{ end }}.
...
- Add `http.go`,{{ if .Computed.enable_grpc_final }} `grpc.go`,{{ end }} or other files only when the capability actually gains that boundary.

## Dependencies

- Accept required dependencies explicitly through constructors or handler-maker functions; avoid package globals.
- Pass `context.Context` through database, Redis,{{ if .Computed.enable_grpc_final }} gRPC,{{ end }} and downstream HTTP operations.
