...
func TestHTTP(t *testing.T) {
	...
	router, err := server.NewRouter(cfg, idempotency.NewMemoryStore(), m)
	if err != nil {
		t.Fatal(err)
	}
	...
	if date := page["items"].([]any)[0].(map[string]any)["created_at"]; date != float64(testTime.UnixMilli()) {
	...
	if body := request("GET", "/game?p=3&s=1", "", 200).Body.String(); !strings.Contains(body, `"items":[]`) || !strings.Contains(body, `"t":2`) {
		t.Fatal(body)
	}
	...
	if value["created_at"] != float64(testTime.UnixMilli()) || value["updated_at"] != value["created_at"] {
...
func TestLocalizedGameErrors(t *testing.T) {
	m, _ := newTestModule(t)
	cfg := &config.Config{}
	cfg.Idempotency.TTL = time.Hour
	router, err := server.NewRouter(cfg, idempotency.NewMemoryStore(), m)
	...
	for _, language := range []string{"en-US", "zh-CN"} {
		...
		want := "invalid game id"
		if language == "zh-CN" {
			want = "游戏 ID 无效"
		}
		if response.Code != 400 || body.ErrorCode != "GAME_INVALID_ID" || body.Msg != want {
			t.Fatalf("%s: %d %s", language, response.Code, response.Body.String())
		}
	}
}
