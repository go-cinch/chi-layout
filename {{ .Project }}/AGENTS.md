...
- `internal/common` must not import concrete business packages under `internal/modules/<capability>`. HTTP server infrastructure may depend on the interfaces defined in `internal/modules`: `HTTPModule` (also aliased as `Module`) and `IdempotentHTTPModule`.{{ if .Computed.enable_grpc_final }} RPC server infrastructure may depend on `GRPCModule` in the same package.{{ end }}
...
- Name event timestamps consistently with a past participle followed by `At`: `CreatedAt`, `UpdatedAt`, `StartedAt`, and `EndedAt` in Go. Use `created_at`, `updated_at`, `started_at`, and `ended_at` in HTTP JSON, SQL{{ if .Computed.enable_grpc_final }}, and Proto{{ end }}. Do not mix these with `starts_at`/`ends_at` or `start_at`/`end_at`. Apply this convention to requests, responses, configuration, examples, and tests.
...
- Package-level error variables outside `internal/common/apperror` use an `Err` prefix when exported (for example `game.ErrNotFound`) and an `err` prefix when unexported. Error variables inside `apperror` omit the prefix (for example `apperror.InvalidBody` and `apperror.NotFound`). Local error results continue to use `err` or descriptive names such as `parseErr`.
- Public error codes use uppercase snake case and describe both the scope and the failure, such as `GAME_INVALID_ID` or `GAME_NOT_FOUND`; avoid field-only codes such as `GAME_ID`. Keep translation catalog keys, tests, and API documentation in sync when codes change.

## Go Style

- Separate Go function and method declarations with exactly one blank line; add a missing blank line and reduce multiple blank lines to one. Keep documentation comments attached to the function they describe, with the separating blank line before the comments. Short function bodies may remain on one line, such as `func (*Module) Name() string { return "/game" }`. Preserve intentional whitespace in string literals and test fixture source. This is a source style convention, not an additional lint check.
...
- Modules opt in to POST duplicate prevention by implementing `modules.IdempotentHTTPModule` and returning true from `Idempotent()`. The router atomically claims non-empty `x-idempotent` keys before the handler; repeated keys return HTTP 409. Missing/empty keys bypass claims. Claims remain consumed until the configured TTL even if the handler fails; responses are not replayed. Keys are shared across opted-in endpoints within a service deployment, so use a unique key per logical operation. Redis shares claims across replicas; the memory fallback is process-local.
...
- Define expected business errors in their owning capability with `apperror.New(code, diagnosticEnglish)`; compare errors with `errors.Is`/`errors.As`, never translated messages. Keep both `en-US` and `zh-CN` catalogs in sync. Negotiate `Accept-Language` at the HTTP boundary and return `Content-Language` plus `Vary: Accept-Language`; unsupported languages fall back to English. Logs and user content are not translated. Unexpected errors expose only the generic internal error.
...
{{ end -}}
HTTP uses singular resource paths, `p`/`s`, `t` for the total count, and comma-separated IDs for bulk deletion.

### Pagination

- Use `p` (page number) and `s` (page size) for pagination fields in HTTP{{ if .Computed.enable_grpc_final }} and Proto{{ end }} requests/responses. Request fields are optional; omission selects defaults.
...
- Configure the shared Redis key prefix with `redis.prefix` in `conf/redis.yml` or `SERVICE_REDIS_PREFIX`. Use `<environment>:<service>:`; the default is `dev:{{ .Scaffold.Project }}:` using the generated Project name verbatim, independently of `service_name` and `module_name`. All replicas of the same deployment must use the same prefix; other services and environments must use distinct prefixes.
